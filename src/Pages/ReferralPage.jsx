...
const ReferralPage = () => {
    ...
    const handleInviteClick = () => {
        ...
        const message = `🚀 Join me on Bounty Tap and start earning rewards!\n\n🔗 App Link: ${appInviteLink}?referralCode=${referralCode}\n\n💰 Use my referral code to get a bonus of 1000 coins: ${referralCode}`;

        if (window.Telegram?.WebApp) {
            // Open the Telegram share link directly
            window.Telegram.WebApp.openLink(`https://t.me/share/url?url=${encodeURIComponent(appInviteLink)}&text=${encodeURIComponent(message)}`);
        } else {
