...
const generateReferralCode = () => {
  ...
  const showReferralPrompt = (userId) => {
    // Show popup with input field for referral code
    const hasReferral = window.confirm("Do you have a referral code?");

    if (hasReferral) {
        const referralCode = prompt("Enter the referral code:");

        if (referralCode) {
            handleNewUserWithReferral(userId, referralCode);
        }
    } else {
        // User does not have a referral code, create new user normally
